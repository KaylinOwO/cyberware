...
#include "../../helpers/rendering/drawmanager.h"
...
namespace GrenadePrediction {
	...
	void draw() {
		if (!g_Options.GrenadeIndicator || !g_LocalPlayer) return;
		if (g_LocalPlayer->IsDormant() || !g_LocalPlayer->IsAlive()) return;
		Vector start, end;
		...
		if (_predicted) {
			for (auto& p : _points) {
				...
				if (valid && valid2)
					gDrawManager.DrawLine(start.x, start.y, end.x, end.y, g_Options.color_grenade_indicator);
			}
		}
	}
	...
	void trace(CUserCmd* ucmd) {
		if (!g_Options.GrenadeIndicator || !(ucmd->buttons & IN_ATTACK) && !(ucmd->buttons & IN_ATTACK2)) {
			_predicted = false;
			return;
		}
